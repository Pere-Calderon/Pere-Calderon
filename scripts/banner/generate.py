...
ROWS = [
    ...
    ("Education", "Computer Engineering · UdG"),
    ("Status", "Open to work + Learning + Shipping"),
    ("ToolChain", "VS Code · Docker · Git"),
    ("Core.Lang", "Python · TypeScript · Java · C++"),
    ("Core.Frontend", "React · Next.js · React Native · Tailwind"),
    ("Core.Backend", "FastAPI · NestJS · Node"),
    ("Core.AI", "RAG · LLMs · Function Calling · OCR"),
    ("Core.Database", "PostgreSQL · pgvector · Supabase"),
    ("Grid.Mail", "perecalderonlanzaran@gmail.com"),
    ("Grid.LinkedIn", "/in/pere-calderon-7080462b8"),
    ("Grid.GitHub", "Pere-Calderon"),
    ("Grid.Instagram", "@caalde04"),
]
...
def make_logos() -> dict[str, Image.Image]:
    ...
    # Gear: twelve teeth, heavy annulus, and hub cutout (backend / systems).
    ...
    # Neural network: three layers of nodes (3-4-3) fully connected (AI).
    neural = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(neural)
    layers = [
        [(70, 120), (70, 200), (70, 280)],
        [(200, 80), (200, 160), (200, 240), (200, 320)],
        [(330, 120), (330, 200), (330, 280)],
    ]
    ...
    for layer in layers:
        for x, y in layer:
            d.ellipse((x - 26, y - 26, x + 26, y + 26), fill="black")
    logos["neural"] = neural
...
def portrait_points(theme: str, rng: np.random.Generator) -> np.ndarray:
    ...
    if theme == "dark":
        # Autocontrast + gamma instead of equalize: keeps the dark gown dark so
        # the face and shirt carry the detail and the dot count stays under cap.
        mask = Image.fromarray(np.uint8((alpha > 0.08) * 255), "L")
        prepared = ImageOps.autocontrast(prepared, cutoff=1, mask=mask)
        gamma = np.asarray(prepared, dtype=np.float32) / 255.0
        prepared = Image.fromarray(np.uint8((gamma ** 1.6) * 255), "L")
    else:
...
def render_svg(
    ...
    gear = transport(source, logo_points["gear"][:n])
    code = transport(gear, logo_points["code"][:n])
    neural = transport(code, logo_points["neural"][:n])
    ...
    frames = [source, source, gear, gear, code, code, neural, neural, source]
    ...
    parts: list[str] = [
        ...
        "<title id=\"title\">Pere's live system profile</title>",
        '<desc id="desc">Animated terminal profile with a dithered portrait and '
        "gear, code, and neural-network silhouettes.</desc>",
        "<defs>",
    ...
    # Dense portrait drift: 94 independently noisy bands moving toward the gear's centroid.
    gear_centroid = gear.mean(axis=0)
    ...
    for band in range(94):
        ...
        delta = (gear_centroid - centroid) * 0.18 + noise[band]
        d = point_path(pts)
        parts.append(
    ...
    parts.extend(
        [
            ...
            'font-weight="700">@Pere-Calderon</text>',
        ]
    )
...
def main() -> None:
    ...
    for index, theme in enumerate(THEMES):
        ...
        output = ASSETS / f"banner-{theme}.v1.svg"
